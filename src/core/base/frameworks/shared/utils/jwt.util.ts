...
export const generateExpiredDateToken = (): Date => {
  const expIn = appConfig.JWT_EXPIRES_IN as StringValue;

  return new Date(Date.now() + ms(expIn));
};

export const generateExpiredDateRefresh = (): Date => {
  const expIn = appConfig.JWT_REFRESH_EXPIRES_IN as StringValue;
...
export const generateJwt = async ({
  ...
  const token = jwt.sign(
    ...
    {
      expiresIn: appConfig.JWT_EXPIRES_IN as StringValue,
